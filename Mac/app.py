        ...
        try:
            ...
            if use_llm:
                ...
                try:
                    ...
                    yield sse("Ensure ollama is running and 'qwen2.5:14b' model is available.", event="zt_error")
                    # Continue without LLM - we already have the HTML report

                if report_text:
                    # Save the markdown report
                    llm_file = f"zerotrust_ai_report_{timestamp}.md"
                    llm_path = os.path.join(client_folder, llm_file)
                    with open(llm_path, "w") as f:
                        f.write(report_text)
        ...
        try:
            ...
            if use_llm:
                ...
                try:
                    ...
                    yield sse("Ensure ollama is running and 'qwen2.5:14b' model is available.", event="ari_error")
                    # Continue without LLM - we already have the HTML report
